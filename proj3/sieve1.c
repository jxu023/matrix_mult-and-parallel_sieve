...
int main (int argc, char *argv[])
{
	...
	do {
		...
		// process 0 finds one prime and then broadcasts
		if (!id) {
			while (marked[++index]);
			prime = index*2 + 3;
		}
		// I assume this means other processes will block on this until
		// it receives a broadcast which changes the value of prime
		if (p > 1) {
